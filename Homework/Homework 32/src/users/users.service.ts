...
export class UsersService {
  ...
  /**
   * "Deleting" an account deactivates it rather than removing the row, so
   * an email notification has somewhere to point and the account can still
   * be investigated/restored later.
   */
  async deactivate(id: number) {
    ...
    await this.emailSenderService.sendAccountDeactivatedEmail(
      user.email,
      user.firstName,
    );
  ...
  findByEmailWithOtp(email: string) {
    return this.usersRepository
      .createQueryBuilder('user')
      .addSelect(['user.otpCode', 'user.otpCodeExpiresAt'])
      .where('user.email = :email', { email: email.toLowerCase() })
      .getOne();
  }

  async setOtp(id: number, otpCode: string, otpCodeExpiresAt: Date) {
    await this.usersRepository.update(id, { otpCode, otpCodeExpiresAt });
  }
